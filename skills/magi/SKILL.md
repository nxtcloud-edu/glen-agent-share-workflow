...
```bash
magi                  # cwd 프로젝트의 3노드 기동 + 워처 + 캐스퍼 세션 attach
magi status           # 기동 없이 상태만 (워처 상태 포함)
magi init             # 워크트리·저널 셋업만
magi watch stop       # 워처 중지 (start|stop|status)
...
역핑은 실전에서 누락된다(Gotcha 11). 캐스퍼가 유휴여도 코더의 상태 변화를 놓치지 않도록 `magi`가 노드 기동 직후 백그라운드 워처를 띄운다(`--no-watch`로 해제). **노드별 독립 3신호**로 감시한다 — 하나가 조용해도 나머지는 계속 돈다(Gotcha 3: AND 조건 금지):

| 신호 | 잡는 것 |
|---|---|
| 커밋 팁 변화 | 완료 가능성 — **주 신호** |
| 세션 소멸 | 노드 죽음 — 커밋 없이 죽으면 팁 폴링으로는 영원히 안 잡힌다 |
| 승인 프롬프트 | 입력 대기로 멈춘 정지 상태 |

알림은 캐스퍼 세션에 send-keys하는 **wake-up 신호일 뿐**이다 — 핑과 같은 규약이라 캐스퍼는 깨어나면 저널·팁·pane을 직접 실측한다. 캐스퍼 세션이 사라지면 알릴 대상이 없으므로 워처도 물러난다.

유휴와 정체를 가르는 시간 기반 판정은 **넣지 않았다** — standby 노드에서 오탐만 낳는다. 정지 상태는 승인 프롬프트 신호로 잡고, 나머지는 `magi status`로 사람이 실측한다.
